...
int main(int argc, char *argv[])
{
	...
	// float x_init = current_pose.position.x;
	...
	for (int i = 0; i < num_samples; i++){
		...
		double width = 0.2;
		double height = 0.3;
		double t = 2.0 * pi / num_samples * (i + 1);
		current_pose.position.y = 2 * y_init - (width * std::cos(t) / (1 + std::sin(t) * std::sin(t)) + y_init - width);
		current_pose.position.z = z_init + std::sin(t) * std::cos(t) / (1 + std::sin(t) * std::sin(t)) * height;

		double radius = std::sqrt(1.2841 * 1.2841 + 0.15968 * 0.15968);
		current_pose.position.x = std::sqrt(pow(radius, 2) - pow(current_pose.position.y, 2) - pow(current_pose.position.z - 1.0249, 2));
    ...
    for (unsigned i=0; i< trajectory_velocity_changed.joint_trajectory.points.size(); i++)
    {
		...
		{
			std::ofstream outfile ("/home/wenda/powerarm_ws/bernoulli_points.txt",std::ios::out);
			if(!outfile.is_open())
			{
