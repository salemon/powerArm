...
try:
    ...
    ct = time.time()
    q = queue.Queue()
    a = 50 # max motor torq acc
    mt = 260 # motor torq max
    ...
    while True:
        ...
        for i in range(4):
            pred_curr[i] = torq2curr(-(np.sign(diff[i]) * (abs(diff[i]) - dead_zone[i])) if abs(diff[i]) > dead_zone[i] else 0, i)
            pred_curr[i] = round(prev_tau[i]*6.7 + max(min(pred_curr[i], a), -a)) 
            pred_curr[i] = max(min(pred_curr[i], mt), 0)
            command += to_char(round(pred_curr[i]))
