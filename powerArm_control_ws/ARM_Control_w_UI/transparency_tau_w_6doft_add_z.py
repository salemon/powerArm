...
encOffset = [157, -270, 11000, -300]  ## 4. 115020 -- 105720
...
try:
    ...
    dead_zone = np.array([0, 2, 0.10, 0.1])
    while True:
        line = ser.readline()
        data = line.decode("utf-8")
        data_ls = data.split(",")
        ...
        torque_feedforward = -1600
        #torque feedforward
        if (abs(raw_q_dot[2]) < 5 and abs(pred_curr[2]) > 0):
